...
Input: userQuery (string), petID (integer)
Output: chatbotResponse, sources, emergencyFlag

1:  petProfile ← Database.GetPetProfile(petID)
2:  medicalHistory ← Database.GetHealthJournal(petID)
3:  contextualQuery ← userQuery + "[Pet: " + petProfile.breed + 
                      ", Age: " + petProfile.age + "]"
4:  queryEmbedding ← SentenceTransformer.Encode(contextualQuery)
5:  relevantChunks ← ChromaDB.Search(queryEmbedding, k=2)
6:  knowledgeContext ← ""
7:  sources ← []
8:  for i in range (1, len(relevantChunks)) do
9:      knowledgeContext ← knowledgeContext + relevantChunks[i].text
10:     sources.APPEND(relevantChunks[i].metadata.source)
11: end for
12: prompt ← systemPrompt + knowledgeContext + 
              FormatPetProfile(petProfile) + 
              FormatHistory(medicalHistory) + userQuery
13: llmResponse ← GroqAPI.ChatCompletion(
                   model="llama-3.3-70b-versatile",
                   prompt=prompt, max_tokens=384, temperature=0.7)
14: chatbotResponse ← llmResponse.choices[0].message.content
15: emergencyKeywords ← ["poisoning", "bleeding", "seizure", 
                          "difficulty breathing", "unconscious"]
16: emergencyFlag ← false
17: for each keyword in emergencyKeywords do
18:     if chatbotResponse.Contains(keyword) OR 
           userQuery.Contains(keyword) then
19:         emergencyFlag ← true
20:         break
21:     end if
22: end for
23: Database.LogConversation(petID, userQuery, 
                             chatbotResponse, sources)
24: return (chatbotResponse, sources, emergencyFlag)
\end{verbatim}

\textbf{Complexity \& Optimizations:} O(n + log m) where n=query length, m=database size (281,438). Persistent FastAPI service keeps models loaded, k=2 retrieval reduces overhead, max\_tokens=384 limits inference time, HTTP communication eliminates process spawning (6-9s saved). Achieved: 2-3s average response (8x improvement).
...
5:  resizedImage ← ResizeImage(image, 224x224)
...
\textbf{Model Details:} ConvNeXt V2 with ImageNet pre-training. Dog model: Stanford Dogs (20,580 images, 120 breeds, 50 epochs, 87\% accuracy). Cat model: Oxford-IIIT Pets (7,349 images, 37 breeds, 50 epochs, 85\% accuracy). Data augmentation: rotation, flip, zoom, color jitter.
...
\textbf{Implementation:} OpenStreetMap Nominatim API (10km radius, expandable to 50km if <3 results), Haversine distance for accurate sorting, Firebase Cloud Messaging for reminders, PostgreSQL for availability queries.
...
LangChain v0.1.0 & Python framework for LLM application development & Building RAG pipeline with document loaders, embeddings, and chains & \texttt{RetrievalQA.from\_chain\_type()} \\ \hline

ChromaDB v0.4.22 & Open-source vector database for embeddings & Storing and querying 281,438 veterinary knowledge embeddings & \texttt{collection.query(query\_ embeddings, n\_results=2)} \\ \hline

Sentence Transformers v2.2.2 & Pre-trained models for generating sentence embeddings & Creating semantic embeddings for chatbot queries and knowledge chunks & \texttt{model.encode(sentences)} with all-MiniLM-L6-v2 \\ \hline

TensorFlow v2.13 & Machine learning framework & Training and serving ConvNeXt V2 breed identification models & \texttt{model.predict()} for inference \\ \hline

GORM v1.25 & Go Object-Relational Mapping library & Database access abstraction for PostgreSQL & \texttt{db.Create()}, \texttt{db.Find()}, \texttt{db.Updates()} \\ \hline

Gin v1.9 & High-performance Go HTTP web framework & Building RESTful API endpoints with routing and middleware & \texttt{router.POST()}, \texttt{router.Use(middleware)} \\ \hline

Redis Go Client v9 & In-memory data store client for Go & Caching pet profiles, session tokens, rate limiting & \texttt{rdb.Set()}, \texttt{rdb.Get()}, \texttt{rdb.Expire()} \\ \hline

JWT-Go v5 & JSON Web Token implementation for Go & User authentication and authorization & \texttt{jwt.NewWithClaims()}, \texttt{jwt.Parse()} \\ \hline

Bcrypt Go & Password hashing library & Secure password storage with cost factor 12 & \texttt{bcrypt.GenerateFromPassword()}, \texttt{bcrypt.CompareHashAndPassword()} \\ \hline
...
\hline
\textbf{Test ID} & \textbf{Test Objective} & \textbf{Precondition} & \textbf{Steps} & \textbf{Test Data} & \textbf{Expected Result} & \textbf{Post-condition} & \textbf{Actual Result} & \textbf{Pass/Fail} \\ \hline
1 & Verify dog breed identification accuracy & CNN model loaded & 1. Upload dog image 2. Preprocess to 224x224 3. Run inference & Golden Retriever image (3024x4032 JPG) & Top-1 prediction: Golden Retriever (confidence ≥85\%) & Breed saved to profile & Predicted correctly, 92\% confidence & Pass \\ \hline
...
3 & Verify image preprocessing pipeline & None & 1. Input various image sizes 2. Resize to 224x224 3. Normalize [-1,1] & Images: 480x640, 1024x768, 3024x4032 & All resized correctly, normalized & Preprocessed tensor ready & All images processed correctly & Pass \\ \hline
4 & Verify top-3 predictions returned & Model loaded & 1. Run inference 2. Get top-3 breeds & Mixed breed dog image & Returns 3 breeds with confidence scores & Top-3 displayed & Returned: Labrador (78\%), Golden Retriever (65\%), Beagle (54\%) & Pass \\ \hline
5 & Verify invalid image format rejection & None & 1. Upload invalid file 2. Validate format & File: document.pdf & System rejects with error & No processing attempted & Error: "Invalid format, accept JPG/PNG/HEIC" & Pass \\ \hline
6 & Verify oversized image rejection & None & 1. Upload large file 2. Check size limit & Image: 15MB JPEG & System rejects "File too large" & No processing attempted & Error: "Max size 10MB", HTTP 413 & Pass \\ \hline
...
7 & Verify response time optimization & FastAPI persistent service & 1. Send 10 queries 2. Measure HTTP mode 3. Compare to exec baseline & 10 varied queries & HTTP mode 8x faster than exec & Performance improvement & HTTP avg: 2.9s, Exec avg: 23.4s (8.1x faster) & Pass \\ \hline
8 & Verify knowledge source citation & RAG pipeline active & 1. Retrieve chunks 2. Extract metadata 3. Return sources & Query with retrieved context & Response includes source citations & Sources displayed & Cited 2 sources: ASPCA, Healthy Pets & Pass \\ \hline
\end{tabular}
...
3 & Verify GORM ORM operations & Database connected & 1. Create record 2. Query record 3. Update record 4. Delete record & Pet entity with all fields & All CRUD operations successful & Record lifecycle managed & Created, queried, updated, deleted successfully & Pass \\ \hline
4 & Verify API rate limiting middleware & Gin router configured & 1. Send 101 requests in 1 min 2. Check 101st request & 100 requests/minute limit per user & Request 101 rejected with HTTP 429 & Rate limit enforced & 101st request rejected: "Too Many Requests" & Pass \\ \hline
5 & Verify CORS configuration & API server running & 1. Send preflight OPTIONS 2. Check headers 3. Send actual request & Cross-origin request from frontend & CORS headers present, request allowed & Request processed & Headers: Access-Control-Allow-Origin: *, request allowed & Pass \\ \hline
6 & Verify error logging & Logger initialized & 1. Trigger error 2. Check log file 3. Verify stack trace & Database connection error & Error logged with timestamp, level, stack trace & Log entry created & Logged: ERROR level, full stack trace, timestamp & Pass \\ \hline
7 & Verify JWT middleware authentication & Protected route configured & 1. Send request without token 2. Send with invalid token 3. Send with valid token & Valid/invalid JWT tokens & No token: 401, Invalid: 401, Valid: 200 & Authentication enforced & All cases handled correctly & Pass \\ \hline
8 & Verify input validation middleware & Validation rules defined & 1. Send invalid data 2. Check validation errors 3. Send valid data & Invalid: email="notanemail", Valid: email="user@test.com" & Invalid rejected with error, Valid accepted & Validation working & Invalid rejected: "Invalid email format", Valid accepted & Pass \\ \hline
\end{tabular}
